...
**Status: Phases 1-5 done.** Real send/attest/prove/verify round trip
...
(Phase 4). `ProofGate` is now hardened — emitter allowlist, topic0/topic-count
...
(Phase 5). See [`spikes/FINDINGS.md`](./spikes/FINDINGS.md) for details.
...
| `contracts/creditcoin/` | Foundry project for Creditcoin CC3 (`ProofGate.sol` hardened; `FactionMarch.sol`, `WarChest.sol` upcoming) |
